    ...
    match &migration.root {
        Some(root) => {
            assert!(root.children.is_empty(),
                "identical files should produce no diff children, got: {:?}", root.children);
        }
...
fn test_added_file() {
    ...
    let added = root.children.iter().find(|c| c.kind == "add").expect("should have add node");
...
fn test_removed_file() {
    ...
    let root = migration.root.expect("should have root");
    let removed = root.children.iter().find(|c| c.kind == "remove").expect("should have remove node");
...
fn test_modified_text_file() {
    ...
    fs::write(dir_b.join("notes.txt"), "line 1\nline 2 modified\nline 3\nline 4\n").unwrap();
    ...
    let root = migration.root.expect("should have root");
    let modified = root.children.iter().find(|c| c.kind == "modify").expect("should have modify node");
    assert_eq!(modified.item_type, "text");
    assert!(modified.tags.contains("binoc.content-changed"));

    let lines_added = modified.details.get("lines_added").unwrap().as_u64().unwrap();
    let lines_removed = modified.details.get("lines_removed").unwrap().as_u64().unwrap();
...
fn test_csv_column_changes() {
    ...
    fs::write(dir_a.join("data.csv"), "name,age,city\nAlice,30,NYC\nBob,25,LA\n").unwrap();
    fs::write(dir_b.join("data.csv"), "name,age,city,email\nAlice,30,NYC,a@b.com\nBob,25,LA,b@c.com\nCharlie,35,SF,c@d.com\n").unwrap();
    ...
    let root = migration.root.expect("should have root");
    let csv_node = root.children.iter()
    ...
    let cols_added = csv_node.details.get("columns_added").unwrap().as_array().unwrap();
...
fn test_csv_column_reorder_only() {
    ...
    fs::write(dir_a.join("data.csv"), "name,age,city\nAlice,30,NYC\nBob,25,LA\n").unwrap();
    fs::write(dir_b.join("data.csv"), "city,name,age\nNYC,Alice,30\nLA,Bob,25\n").unwrap();
...
fn test_move_detection() {
    ...
    assert!(move_node.is_some(), "should detect move, got: {:?}",
...
fn test_zip_comparison() {
    ...
    let root = migration.root.expect("should have root");
    let zip_node = root.children.iter()
        .find(|c| c.item_type == "zip_archive")
        .expect("should have zip_archive node");

    assert!(!zip_node.children.is_empty() ||
        zip_node.children.iter().any(|c| !c.children.is_empty()),
